...
import { Plus, Filter, LayoutGrid, List as ListIcon, MoreVertical } from 'lucide-react';
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 hover:border-slate-600 transition-colors group relative">
      <div className="flex justify-between items-start mb-4">
        <div>
          <h3 className="text-2xl font-bold text-white font-display">{room.room_number}</h3>
          <p className="text-sm text-slate-400 capitalize">{room.room_type.replace('-', ' ')}</p>
        </div>
        <span className={`px-2 py-1 rounded text-xs font-semibold uppercase tracking-wide border ${statusColors[room.current_status]}`}>
          {room.current_status}
        </span>
      </div>
      ...
      <div className="pt-4 border-t border-slate-800 grid grid-cols-2 gap-2">
        ...
        {room.current_status === 'available' && (
         ...
         {(room.current_status === 'occupied' || room.current_status === 'maintenance') && (
          <button 
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm">
      <div className="bg-slate-900 border border-slate-700 w-full max-w-md rounded-2xl p-6 shadow-2xl">
        <h2 className="text-xl font-display font-bold text-white mb-6">Create New Room</h2>
        <form onSubmit={handleSubmit} className="space-y-4">
            <div className="grid grid-cols-2 gap-4">
                <div>
                    <label className="text-xs text-slate-400 mb-1 block">Room Number</label>
                    <input required className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2.5 text-white focus:border-primary-500 outline-none" 
                ...
                <div>
                    ...
                    <input type="number" required className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2.5 text-white focus:border-primary-500 outline-none" 
            ...
            <div className="grid grid-cols-2 gap-4">
                <div>
                    ...
                    <select className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2.5 text-white outline-none"
                ...
                <div>
                    ...
                    <select className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2.5 text-white outline-none"
                        value={formData.viewType} onChange={e => setFormData({...formData, viewType: e.target.value})}>
            ...
            <div>
                <label className="text-xs text-slate-400 mb-1 block">Base Rate (₹)</label>
                <input type="number" required className="w-full bg-slate-950 border border-slate-700 rounded-lg p-2.5 text-white focus:border-primary-500 outline-none" 
                    value={formData.basePrice} onChange={e => setFormData({...formData, basePrice: e.target.value})} />
            </div>
            <div className="pt-4 flex gap-3">
                <button type="button" onClick={onClose} className="flex-1 py-2.5 bg-slate-800 text-slate-300 rounded-lg hover:bg-slate-700">Cancel</button>
                <button type="submit" disabled={loading} className="flex-1 py-2.5 bg-primary-600 text-white rounded-lg hover:bg-primary-500">
...
const Rooms = () => {
  ...
  const updateStatus = async (id, status) => {
    await supabase.from('rooms').update({ current_status: status }).eq('id', id);
    fetchRooms();
  };

  const filteredRooms = filter === 'all' ? rooms : rooms.filter(r => r.current_status === filter);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row justify-between gap-4">
        ...
        <button onClick={() => setIsModalOpen(true)} className="flex items-center gap-2 px-4 py-2 bg-primary-600 hover:bg-primary-500 text-white rounded-lg text-sm font-medium transition-colors shadow-lg shadow-primary-900/20">
      ...
      <AddRoomModal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} onSuccess={fetchRooms} />
    </div>
  );
};
