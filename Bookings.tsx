      ...
      <div className="bg-slate-900 border border-slate-700 w-full max-w-lg rounded-2xl p-6 shadow-2xl">
        ...
        <div className="space-y-4">
          ...
          <button
            ...
            className="w-full mt-4 py-2.5 bg-primary-600 text-white rounded-lg hover:bg-primary-500"
          >
            Confirm Booking
          </button>
          <button
...
const Bookings = () => {
  ...
  const fetchBookings = async () => {
    const { data } = await supabase
      .from("reservations")
      .select(
        `*, guests(first_name, last_name, email), rooms(room_number, room_type)`,
      )
          ...
          <tbody className="divide-y divide-slate-800 text-sm text-slate-300">
            {filtered.map((b) => (
              ...
              >
                ...
                <td className="p-4">
                  {format(parseISO(b.check_in_date), "MMM dd, yyyy")}
                </td>
                <td className="p-4">
                  {format(parseISO(b.check_out_date), "MMM dd, yyyy")}
                </td>
                <td className="p-4">
                  <span
                ...
                <td className="p-4 text-right">
                  <div className="flex justify-end gap-2">
                    {b.status === "confirmed" && (
                      <button
                        onClick={() =>
                          updateStatus(b.id, "checked_in", b.room_id)
                        }
                        title="Check In"
                    ...
                    {b.status === "checked_in" && (
                      <button
                        onClick={() =>
                          updateStatus(b.id, "checked_out", b.room_id)
                        }
                        title="Check Out"
