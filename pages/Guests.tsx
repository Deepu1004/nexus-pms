  ...
  return (
    <div className="space-y-6">
       ...
       <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filtered.map(guest => (
             <div key={guest.id} className="bg-slate-900 border border-slate-800 rounded-xl p-5 hover:border-primary-500/50 transition-colors group">
                ...
                <h3 className="text-white font-semibold text-lg">{guest.first_name} {guest.last_name}</h3>
                
                <div className="mt-4 space-y-2">
                   <div className="flex items-center gap-2 text-sm text-slate-400">
                      <Mail className="w-3.5 h-3.5" /> <span className="truncate">{guest.email || 'No email'}</span>
                   </div>
                   <div className="flex items-center gap-2 text-sm text-slate-400">
                      <Phone className="w-3.5 h-3.5" /> <span>{guest.phone || 'N/A'}</span>
                   </div>
                   <div className="flex items-center gap-2 text-sm text-slate-400">
                      <MapPin className="w-3.5 h-3.5" /> <span>{guest.nationality || 'Unknown'}</span>
                   </div>
                </div>

                <div className="mt-4 pt-4 border-t border-slate-800 flex justify-between text-xs text-slate-500">
                   <span>Visits: <span className="text-slate-300">{guest.visit_count}</span></span>
                   <span>Spent: <span className="text-slate-300">₹{guest.total_spent}</span></span>
                </div>
             </div>
          ))}
       </div>
