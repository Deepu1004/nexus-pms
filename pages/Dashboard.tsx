...
import { 
  Users, DollarSign, BedDouble, TrendingUp, CalendarCheck, 
...
const StatCard = ({ title, value, subtext, icon: Icon, trend }) => (
  <div className="glass-card p-6 rounded-xl border border-slate-800 bg-slate-900/50">
    ...
    <div>
      ...
      <p className="text-2xl font-display font-semibold text-white mt-1">{value}</p>
...
const Dashboard = () => {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const { count: occupied } = await supabase.from('rooms').select('*', { count: 'exact', head: true }).eq('current_status', 'occupied');
      
      const occupancy = totalRooms ? Math.round((occupied || 0) / totalRooms * 100) : 0;
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2 glass-card p-6 rounded-xl border border-slate-800">
          <h3 className="text-lg font-display font-semibold text-white mb-6">Revenue Performance</h3>
        ...
        <div className="glass-card p-6 rounded-xl border border-slate-800 flex flex-col">
          <h3 className="text-lg font-display font-semibold text-white mb-4">System Notices</h3>
          <div className="space-y-4 flex-1 overflow-y-auto">
             <div className="p-3 bg-rose-500/10 border border-rose-500/20 rounded-lg">
                <p className="text-xs text-rose-400 font-bold uppercase mb-1">Action Required</p>
                <p className="text-sm text-slate-300">Room 304 reported AC malfunction. Maintenance ticket created.</p>
             </div>
             <div className="p-3 bg-amber-500/10 border border-amber-500/20 rounded-lg">
                <p className="text-xs text-amber-400 font-bold uppercase mb-1">Housekeeping</p>
                <p className="text-sm text-slate-300">4 rooms pending turnover for 2:00 PM check-ins.</p>
             </div>
             <div className="p-3 bg-emerald-500/10 border border-emerald-500/20 rounded-lg">
                <p className="text-xs text-emerald-400 font-bold uppercase mb-1">System</p>
                <p className="text-sm text-slate-300">Night audit completed successfully at 03:00 AM.</p>
             </div>
          </div>
        </div>
      </div>
