...
const Revenue = () => {
  ...
  useEffect(() => {
    ...
    const loadData = async () => {
      const { data } = await supabase
        .from("payments")
        .select("*, reservations(guests(first_name, last_name))")
  ...
  const chartData = transactions
    .slice(0, 20)
    .map((t) => ({
      date: new Date(t.payment_date).toLocaleDateString(undefined, {
        month: "short",
        day: "numeric",
      }),
      amount: t.amount,
            ...
            {transactions.map((t) => (
              <tr key={t.id} className="hover:bg-slate-800/50">
                ...
                <td className="p-4 text-white font-medium">
                  {t.reservations?.guests?.first_name}{" "}
                  {t.reservations?.guests?.last_name}
                </td>
